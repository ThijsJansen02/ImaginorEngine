...
namespace IME::OpenGL {
    ...
    MemoryArena
    createMemoryArena(byte* data, sizeptr size) {
    ...
    deallocateMemory_(MemoryPool* pool, byte* data, sizeptr size) {
        ...
        IME_DEBUG_ASSERT_BREAK(size > 0 && data != nullptr, "no correct memory deallocation!")
    ...
    IME_DEBUG_ASSERT_BREAK(datasize != 0, "you cant allocate memory of size 0!")
